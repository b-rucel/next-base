...
To add custom components in NextBase, follow these steps:

1. **Create Your Component**: First, create your custom component in the `components/markdown` folder. For example, you might create a file named `CustomAlert.tsx`:
...
export default function CustomAlert({ children, type = 'info' }: CustomAlertProps) {
  return (
    <div className={`my-4 p-4 rounded-lg border ${type}-alert`}>
      {children}
    </div>
  );
}
